...
public class Marshaler implements IRemoteInvocation {
    ...
    public byte[] marshal(UUID interfaceID, String methodName, Object[] params) {
        JSONObject jsonObj = new JSONObject();
        JSONArray paramsJSON = new JSONArray();
     ...
     */
    private String[] cacheOrLookup(UUID interfaceID, String methodName) {
     ...
     * @return JSON-Objekt als byte[] bereit zum Versenden
     */
    private byte[] serializeNS(UUID interfaceID, String methodName) {
    ...
    private void cache(UUID interfaceID, String methodName, String[] ipAddresses) {
        Pair<String, String> key;
        key = new Pair<>(String.valueOf(interfaceID), methodName);
        Set<String> val = new HashSet<>() {{
            for (String ip : ipAddresses) {
