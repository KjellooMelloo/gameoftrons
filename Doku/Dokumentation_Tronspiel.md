...
|UC5, UC7| View |removeTileColor(int) | Ein Spieler ist gestorben und wurde aus dem Spiel entfernt. | Die Felder, die mit als Parameter eingegebenen Farbe eingefärbt waren, sind nicht mehr eingefärbt, sondern haben die gleiche Farbe wie der Spielhintergrund.| Die Methode ändert die Farbe von jedem Feld mit der als Parameter übergebenen Farbe zur Hintergrundfarbe. | |
| UC1-8 | View |updatePlayer(int ID, int X, int Y) |Im Model wurden Daten zu den Spielern geändert. | Die View hat ihre Daten aktualisiert. | Die Methode aktualisiert die Position des Spieler mit der übergebenen ID auf die übergebenen Koordinaten. Wenn die Koordinaten <0 sind, dann ist der Spieler tot und die Methode kill() wird aufgerufen. | |
