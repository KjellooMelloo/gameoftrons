...
|**UC3** Start|Game|Alle bis auf den letzten Mitspieler befinden sich im Warteraum. Der letzte fehlende Spieler betritt den Warteraum.|Das Spiel wurde gestartet und allen Spielern wird der Spielbildschirm angezeigt.|**1.** Das System fügt den letzten Spieler der Spielerliste der Spielinstanz hinzu.<br><br>**2.** Das System zeigt den 3-Sekunden-Countdown an.<br><br>**3.** Das System wechselt zum Spielbildschirm.|||
...
| --- | --- | --- | --- |
| | int getPlayerCount() | Die Methode liefert die durch den Benutzer eingegebenen Spieleranzahl | Wenn die Spieleranzahl keine Zahl zwischen 2 und 6 ist, wird die Methode loadDefaulftPlayerCount() aufgerufen |
| | int loadDefaultPlayerCount() | Die Methode liefert den Default-Wert für die Spieleranzahl aus der Config-Datei. | Wenn keine Zahl geladen werden konnte, wird eine Exception mit Fehlerbeschreibung geworfen. |
| | int loadMaxWaitingTime() | Die Methode liefert die maximale Wartezeit (Einheit: Sekunden) auf anderen Spieler aus der Config-Datei. | Wenn keine Zahl geladen werden konnte, wird eine Exception mit Fehlerbeschreibung geworfen. |
| | int loadGameFieldWidth() | Die Methode liefert die Spielfeldbreite (Einheit: Anzahl Felder) aus der Config-Datei. | Wenn keine Zahl geladen werden konnte, wird eine Exception mit Fehlerbeschreibung geworfen. |
| | int loadGameFieldHeight() | Die Methode liefert die Spielfeldhöhe (Einheit: Anzahl Felder) aus der Config-Datei | Wenn keine Zahl geladen werden konnte, wird eine Exception mit Fehlerbeschreibung geworfen. |
| | int loadGameFieldSquareSize() | Die Methode liefert die Größe eines Feldes im Raster (Einheit: Anzahl Pixel) aus der Config-Datei. Die Größe gilt für die Breite und die Höhe eines Feldes. | Wenn keine Zahl geladen werden konnte, wird eine Exception mit Fehlerbeschreibung geworfen. |
| | int loadPlayerSpeed() | Die Methode liefert die Geschwindigkeit, in der sich das Motorrad der Spieler bewegen soll (Einheit: Anzahl Bewegungen pro Sekunde) aus der Config-Datei. | Wenn keine Zahl geladen werden konnte, wird eine Exception mit Fehlerbeschreibung geworfen. Wenn die Zahl geladen werden konnte, die kleiner als 1 oder größer 500 ist, wird ebenfalls eine Exception mit Fehlerbeschreibung geworfen.|
| | static Game createGameInstance() | Die Methode ruft die oberen Methoden auf und erstellt eine neue Spielinstanz mit den geladenen Parametern. Die Methode gibt true zurück, wenn die Spielinstanz erfolgreich erstellt werden konnte.  Die Methode gibt false zurück, wenn die Spielinstanz nicht erfolgreich  erstellt werden konnte | |
| | Player createPlayer() | Die Methode erstellt einen neuen Spieler, indem dem Spieler eine noch nicht vergebene ID und Farbe vergeben wird. Die Methode gibt den erstellten Spieler zurück | Wenn kein Spieler erstellt werden konnte wird eine Exception mit Fehlerbeschreibung geworfen|
| | void registerPlayer(Player) | Die Methode fügt die als Parameter übergebene Spielerinstanz in die Spielerliste hinzu.| Exception!|
| | void informUSer(String) | Zeigt Fehlerbeschreibung der Exception dem Nutzer an ||
