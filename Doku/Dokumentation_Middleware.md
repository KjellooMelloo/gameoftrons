...
|Methode| Kurzbeschreibung|
| --- | --- |
|register(int interfacIid, IRemoteObject remoteObject, String methodName, InetAddress ipAddr, bool isSingleton)| Packt Aufrufparameter in eine JSON-Nachricht, ruft serializeNS() auf und schickt sie an den Name Server mit den notwendigen Registrierungsdaten. Trägt das RemoteObject unter übergebenen Id in die lokale Tabelle ein. |
|serializeNS(JSON jsonMsg)| Erstellt aus einer Nachricht im JSON-Format ein byte-Array um.|
